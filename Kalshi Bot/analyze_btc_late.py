...
def analyze_btc_late():
    ...
    print("--- BTC Late Momentum (Trigger at 60c with < 5m left) ---")
    for thr in [60, 65, 70]:
        trades = []
        for ticker, group in btc_df.groupby('ticker'):
            ...
            # Look for trigger between 1m and 5m remaining
            late = group[(group['rem'] < 5) & (group['rem'] > 1) & (group['price'] >= thr)]
            if not late.empty:
                # ONLY if it was LOW before (< 40 at 10m)
                early = group[(group['rem'] > 10) & (group['price'] < 40)]
                if not early.empty:
                    trades.append({'outcome': outcome, 'action': 'YES', 'price': late.iloc[0]['price']})
        
        if trades:
            t_df = pd.DataFrame(trades)
            acc = (t_df['outcome'] == 'YES').mean()
            ev = (acc * (100 - t_df['price'].mean())) - ((1 - acc) * t_df['price'].mean())
            print(f"Price > {thr}¢ Late (Late Surge): n={len(t_df)}, Accuracy={acc:.1%}, EV={ev:.1f}¢")
