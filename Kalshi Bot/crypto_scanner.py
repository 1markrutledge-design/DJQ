...
def run_global_grid_search(hours_back):
    ...
    with open(DATA_FILE, newline="", encoding="utf-8-sig") as f:
        ...
        for row in reader:
            ...
            series = parts[0]
            if "15M" not in series: continue
            
            if series not in ["KXBTC15M", "KXSOL15M", "KXXRP15M", "KXDOGE15M"]:
                continue
    ...
    print(f"\n=======================================================")
    print(f" TARGET MARKETS SCANNER (LAST {hours_back} HOURS)")
    print(f"=======================================================")

    for series, series_markets in markets.items():
        valid_tickers = [t for t in series_markets if t in outcomes]
        print(f"\n{series} (Markets: {len(valid_tickers)})")
        ...
        for thresh in range(10, 60, 5):
            ...
            for t in valid_tickers:
                md = sorted(series_markets[t], key=lambda x: x["mins_left"], reverse=True)
                for snap in md:
                    if 1 <= snap["mins_left"] <= 8 and snap["ask"] > 0 and snap["ask"] <= thresh:
                        ...
                        break
            if trades > 2:
                results.append((f"YES Ask <= {thresh:02d} (1-8m)", trades, wins/trades*100, total_entry/trades, total_profit/trades))

        # Grid Search YES Strategies (Momentum)
        for thresh in range(60, 95, 5):
            trades, wins, total_entry, total_profit = 0, 0, 0, 0
            for t in valid_tickers:
                md = sorted(series_markets[t], key=lambda x: x["mins_left"], reverse=True)
                for snap in md:
            ...
            for t in valid_tickers:
                ...
                for snap in md:
                    if 5 <= snap["mins_left"] <= 12 and snap["bid"] > 0 and snap["bid"] <= thresh:
                        ...
                        break
            if trades > 2:
                results.append((f"NO Bid <= {thresh:02d} (5-12m)", trades, wins/trades*100, total_entry/trades, total_profit/trades))

        # Grid Search BTC ZONE SNIPER 
        if series == "KXBTC15M":
            trades, wins, total_entry, total_profit = 0, 0, 0, 0
            for t in valid_tickers:
                md = sorted(series_markets[t], key=lambda x: x["mins_left"], reverse=True)
                for snap in md:
                    if 1 <= snap["mins_left"] <= 12 and 55 <= snap["ask"] <= 75:
                        trades += 1
                        maker = max(1, snap["ask"] - 1)
                        total_entry += maker
                        if outcomes[t] == "YES":
                        ...
                        break
            if trades > 0:
                results.append((f"BTC Zone Sniper Wide", trades, wins/trades*100, total_entry/trades, total_profit/trades))
...
run_global_grid_search(6)
run_global_grid_search(12)
