...
print(f"Total Tennis Tickers traded in last 7 days: {len(tennis_trades)}")
print("-" * 80)
print(f"{'Ticker':<40} | {'P&L ($)':<10} | {'Status'}")
print("-" * 80)
...
for ticker, data in tennis_trades.items():
    ...
    if market_data:
        ...
        if m.get("status") == "finalized":
            ...
            status = f"Open ({m.get('yes_bid', 0)}/{m.get('yes_ask', 0)})"

    pnl_cents = sell_rev + payout - buy_cost
    pnl_dollars = pnl_cents / 100.0
    total_pnl += pnl_dollars
    
    # Always print if there's any activity
    print(f"{ticker:<40} | {pnl_dollars:>10.2f} | {status}")

print("-" * 80)
print(f"Total Combined P&L (last 7 days): ${total_pnl:.2f}")

# Analyze why trades fail
unsuccessful_trades = [t for t, d in tennis_trades.items() if (sum(f.get("yes_price", 0) * f.get("count", 0) for f in d["sells"]) + (0 if not c.get_markets(ticker=t) or c.get_markets(ticker=t)[0].get("result")!="yes" else 100) - sum(f.get("yes_price", 0) * f.get("count", 0) for f in d["buys"]))/100.0 < 0]
print(f"Number of losing tickers: {len(unsuccessful_trades)}")
