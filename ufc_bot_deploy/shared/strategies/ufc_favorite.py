...
class UFCFavoriteStrategy(BaseStrategy):
    ...
    def __init__(self, client):
        ...
        self.buy_price = int(os.getenv("UFC_FAVORITE_BUY_PRICE", "55"))
    ...
    def execute_buyer(self):
        ...
        3. Places limit buy order at 55 if no resting order exists.
        """
        logging.info("Executing UFC Favorite Buyer strategy...")
        
        # 1. Get UFC series markets
        markets = self.client.get_markets(series_ticker="KXUFCFIGHT")
        
        for market in markets:
            ticker = market["ticker"]
            yes_price_dollars = market.get("yes_ask_dollars") or (market.get("yes_ask", 0) / 100.0)
            yes_price = int(float(yes_price_dollars) * 100)
            
            if self.threshold <= yes_price < 99:
                ...
                # 3. Place Limit Order at 55 (Buying 4 shares as requested)
                ...
                try:
                    ...
                    logging.info(f"Placed limit buy for {ticker} at 55 with CID {client_id}")
    ...
    def execute_seller(self):
        ...
        for ticker, qty in strategy_positions.items():
            ...
            # SAFETY CHECK: Only manage UFC tickers
            if "UFC" not in ticker.upper():
                logging.info(f"Skipping non-UFC ticker {ticker} identified with strategy prefix {self.STRATEGY_PREFIX}")
                continue
            ...
            current_price_dollars = market_data[0].get("yes_bid_dollars") or (market_data[0].get("yes_bid", 0) / 100.0)
            current_price = int(float(current_price_dollars) * 100)
            ...
            if shares_to_sell > 0:
                # CHECK FOR RESTING SELL ORDERS: Prevent duplicates
                resting_orders = self.client.get_resting_orders(ticker=ticker)
                ...
                if has_resting_sell:
                    logging.info(f"Skipping {ticker}: Resting sell order already exists. Will check again in next run.")
                    continue
