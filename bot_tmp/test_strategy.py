...
class TestUFCFavoriteStrategy(unittest.TestCase):
    ...
    def test_reconstruction_and_sell(self):
        # 1. Mock fills showing we bought 4 shares of TICKER-A with strategy prefix
        self.mock_client.get_fills.return_value = [
            {"ticker": "TICKER-A", "side": "yes", "count": 4, "client_order_id": "88-uuid1"}
        ]
        
        # 2. Mock market price at 85 (triggering full sell)
        self.mock_client.get_markets.return_value = [{"ticker": "TICKER-A", "yes_bid": 85}]
    ...
    def test_incremental_sell_65(self):
        ...
        self.mock_client.get_markets.return_value = [{"ticker": "TICKER-A", "yes_bid": 65}]
